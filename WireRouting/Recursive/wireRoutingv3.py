...
class Route(object):
	...
	def map_vals(self, x, y):
		...
		zeroBool = False
		for val in n_vals:
			if val > 0 and (currVal - val > 1 or zeroBool): #zeroBool tells us we used an invalid val, so use a valid one
				self.ls[y][x] = val+1
				currVal = val+1
				zeroBool = False #we updated using a valid val
			elif currVal == 0 and val >= 0: #if our currVal is 0 we set it as long as we aren't using an obstacle
				self.ls[y][x] = val+1
				currVal = val+1
				if val == 0:
					zeroBool = True #says we updated our currVal using a 0 (invalid)
		#try to go up
		if y > 0:
